...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 15 / Deep Structured Energy-Based Models audit runner."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        details.update(
            {
                ...
                "implemented_trainable_representatives": [
                    "model15_deep_energy_anomaly_detector",
                ],
                "exact_dsebm_implementation": False,
                "surrogate_implementation": (
                    "model15_deep_energy_anomaly_detector implements a runnable "
                    "reconstruction-energy anomaly detector. It is inspired by "
                    "energy-based anomaly detection, but it is not the exact DSEBM "
                    "training procedure."
                ),
                "remaining_variants_recorded_here": [
                    "exact_deep_structured_energy_based_model",
                    "energy_gradient_based_training",
                    "paper_specific_dsebm_energy_function",
                    "paper_specific_anomaly_scoring_rules",
                ],
                "reason": (
                    "Paper 15 describes Deep Structured Energy-Based Models for "
                    "anomaly detection. The exact method is specialized and not central "
                    "to the final DDoS comparison. A practical reconstruction-energy "
                    "detector is implemented separately, while this audit file preserves "
                    "traceability and clearly marks the exact DSEBM as not reimplemented."
                ),
            }
        )
        ...
        write_audit_only_result(
            ...
            status="audit_only",
            message=(
                "Paper 15 exact DSEBM method recorded as audit-only. "
                "A runnable reconstruction-energy anomaly detector is implemented separately."
            ),
            details=details,
        )
        ...
        print(
            "[audit_only] "
            f"{args.model_id}: Paper 15 exact DSEBM method recorded."
        )
