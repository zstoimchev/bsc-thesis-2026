...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 9 / Subset Feature Elimination remaining variants audit runner."
    )
    ...
    return RunnerArgs(
        ...
        extra={
            ...
            "variant": "remaining_feature_elimination_variants_audit",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        details.update(
            {
                ...
                "implemented_trainable_representatives": [
                    "model09_subset_feature_elimination_decision_tree",
                ],
                "remaining_variants_recorded_here": [
                    "alternative_feature_selection_thresholds",
                    "alternative_classical_ml_classifiers_if_present",
                    "paper_specific_subset_elimination_configurations",
                    "minor_hyperparameter_variants",
                ],
                "reason": (
                    "Paper 9 is represented by an executable ANOVA + RFE feature "
                    "elimination pipeline followed by a Decision Tree classifier. "
                    "This audit file records any remaining feature-selection variants "
                    "without duplicating similar classical ML code."
                ),
            }
        )
        ...
        write_audit_only_result(
            ...
            status="audit_only",
            message=(
                "Paper 9 remaining subset feature elimination variants recorded "
                "as audit-only. The executable ANOVA + RFE + Decision Tree "
                "representative is implemented separately."
            ),
            details=details,
        )
        ...
        print(
            "[audit_only] "
            f"{args.model_id}: Paper 9 remaining variants recorded."
        )
