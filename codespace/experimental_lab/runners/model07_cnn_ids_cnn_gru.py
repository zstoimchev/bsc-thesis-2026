...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 7.1 / CNN-GRU Network Intrusion Detection runner."
    )
...
def reshape_tabular_to_sequence(X):
    """
    Convert tabular IDS flow features into CNN-GRU sequence format.

    Original:
        (samples, features)

    CNN-GRU input:
        (samples, timesteps/features, channels=1)
    """
...
def run_cnn_gru(args: RunnerArgs) -> dict[str, Any]:
    try:
        import tensorflow as tf
    except ImportError:
        return write_audit_only_result(
            ...
            message=(
                ...
                "to execute this Paper 7.1 CNN-GRU runner."
            ),
            details={
    ...
    model_path = artifact_dir / "cnn_gru.keras"
    ...
    metrics.update(
        {
            ...
            "runner": "model07_cnn_ids_cnn_gru",
            "paper": "07.1",
            "repository": "CNN_for_Network_Intrusion_Detection",
            "variant": "cnn_gru",
            "problem_type": args.problem_type,
