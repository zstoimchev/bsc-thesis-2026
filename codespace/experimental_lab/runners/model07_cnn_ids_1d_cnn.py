...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 7.1 / CNN for Network Intrusion Detection 1D CNN runner."
    )
...
def reshape_tabular_to_sequence(X):
    """
    Convert tabular IDS flow features into CNN sequence format.
...
def run_1d_cnn(args: RunnerArgs) -> dict[str, Any]:
    try:
        import tensorflow as tf
    except ImportError:
        return write_audit_only_result(
            ...
            message=(
                ...
                "to execute this Paper 7.1 CNN runner."
            ),
            details={
    ...
    model_path = artifact_dir / "1d_cnn.keras"
