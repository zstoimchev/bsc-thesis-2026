...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 1 / DeepLearning-IDS Keras TensorFlow MLP runner."
    )
    add_common_runner_args(parser)

    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--batch-size", type=int, default=1024)
    parser.add_argument("--layers", default="200,100")
...
def build_keras_mlp(
    ...
    """
    Build a Keras/TensorFlow feedforward MLP.

    This represents the Paper 1 Keras + TensorFlow backend variant.
    """
...
def run_keras_tf_mlp(args: RunnerArgs) -> dict[str, Any]:
    try:
        import tensorflow as tf
    except ImportError:
        return write_audit_only_result(
            ...
            message=(
                ...
                "to execute this Paper 1 Keras TensorFlow MLP runner."
            ),
            details={
                "missing_dependency": "tensorflow",
                "runner": "model01_deeplearning_ids_keras_tf_mlp",
    ...
    y_proba = model.predict(prepared.X_test, batch_size=int(args.extra["batch_size"]))
    ...
    model_path = artifact_dir / "keras_tf_mlp.keras"
