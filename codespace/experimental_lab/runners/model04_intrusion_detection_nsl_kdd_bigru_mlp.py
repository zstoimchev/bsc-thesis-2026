...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 4 / Intrusion-Detection-on-NSL-KDD BiGRU + MLP runner."
    )
    ...
    parser.add_argument("--dense-layers", default="64,32")
...
def reshape_tabular_to_sequence(X):
    """
    Convert tabular features into a simple sequence representation.

    Shape:
        original: (samples, features)
        output:   (samples, features, 1)

    This lets the GRU read the feature vector as a sequence of feature steps.
    """
    X_array = np.asarray(X).astype("float32")
    return X_array.reshape((X_array.shape[0], X_array.shape[1], 1))


def build_bigru_mlp_model(
    timesteps: int,
    num_classes: int,
    gru_units: int,
...
def run_bigru_mlp(args: RunnerArgs) -> dict[str, Any]:
    try:
        import tensorflow as tf
    except ImportError:
        return write_audit_only_result(
            ...
            message=(
                ...
                "to execute this Paper 4 BiGRU + MLP runner."
            ),
            details={
    ...
    model = build_bigru_mlp_model(
        timesteps=timesteps,
        num_classes=num_classes,
        gru_units=int(args.extra["gru_units"]),
    ...
    model_path = artifact_dir / "bigru_mlp.keras"
    ...
    metrics.update(
        {
            ...
            "runner": "model04_intrusion_detection_nsl_kdd_bigru_mlp",
            "paper": "04",
            "repository": "Intrusion-Detection-on-NSL-KDD",
            "variant": "bigru_mlp",
            "problem_type": args.problem_type,
