...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 10 / Clustering anomaly detection remaining variants audit runner."
    )
    ...
    return RunnerArgs(
        ...
        extra={
            ...
            "variant": "remaining_clustering_variants_audit",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        details.update(
            {
                ...
                "implemented_trainable_representatives": [
                    "model10_clustering_anomaly_detection_kmeans",
                ],
                "remaining_variants_recorded_here": [
                    "alternative_cluster_counts",
                    "distance_threshold_variants",
                    "cluster_based_anomaly_score_variants",
                    "paper_specific_unsupervised_configurations",
                ],
                "reason": (
                    "Paper 10 is represented by an executable K-Means clustering "
                    "baseline with majority-label cluster mapping. This audit file "
                    "records remaining clustering or anomaly-scoring variants without "
                    "duplicating similar unsupervised code."
                ),
            }
        )
        ...
        write_audit_only_result(
            ...
            status="audit_only",
            message=(
                "Paper 10 remaining clustering/anomaly-detection variants recorded "
                "as audit-only. The executable K-Means representative is implemented "
                "separately."
            ),
            details=details,
        )
        ...
        print(
            "[audit_only] "
            f"{args.model_id}: Paper 10 remaining variants recorded."
        )
