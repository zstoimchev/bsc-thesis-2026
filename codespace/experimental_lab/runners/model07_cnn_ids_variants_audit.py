...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 7.1 / CNN IDS remaining variants audit runner."
    )
    ...
    return RunnerArgs(
        ...
        extra={
            ...
            "variant": "remaining_cnn_variants_audit",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        details.update(
            {
                ...
                "remaining_variants_recorded_here": [
                    "minor_cnn_configurations_if_present",
                    "minor_hybrid_cnn_rnn_configurations_if_present",
                    "paper_specific_hyperparameter_variants",
                ],
                "reason": (
                    "Paper 7 is represented by executable 1D-CNN, CNN-GRU, and "
                    "CNN-LSTM runners. This audit file records remaining minor CNN "
                    "or hybrid variants without duplicating nearly identical code."
                ),
            }
        )
        ...
        write_audit_only_result(
            ...
            status="audit_only",
            message=(
                "Paper 7 remaining CNN/hybrid variants recorded as audit-only. "
                "Main CNN/CNN-GRU/CNN-LSTM representatives are implemented separately."
            ),
            details=details,
        )
        ...
        print(
            "[audit_only] "
            f"{args.model_id}: Paper 7 remaining variants recorded."
        )
