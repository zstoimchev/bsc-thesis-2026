...
def parse_args(argv: list[str] | None = None) -> RunnerArgs:
    parser = argparse.ArgumentParser(
        description="Paper 11 / Network-Intrusion-Detection-System SVM runner."
    )
    ...
    parser.add_argument(
        "--probability",
        action="store_true",
        help="Enable probability estimates. Slower, but allows probability-based ROC-AUC.",
    )
...
def build_svm_model(args: RunnerArgs) -> SVC:
    return SVC(
        kernel=str(args.extra["kernel"]),
        C=float(args.extra["c"]),
        gamma=str(args.extra["gamma"]),
        degree=int(args.extra["degree"]),
        coef0=float(args.extra["coef0"]),
        class_weight=normalize_optional_none(args.extra["class_weight"]),
        probability=bool(args.extra["probability"]),
